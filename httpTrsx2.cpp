...
static int8_t tcpClient_connect(TRSX *trsx)
{
    int8_t cod_ret;
...
static int16_t tcpClient_getBytesAvailable(TRSX *trsx)
{
    #if defined(__AVR__) && defined(__GNUC__)
...
static int8_t tcpClient_connected(TRSX *trsx)
{
    #if defined(__AVR__) && defined(__GNUC__)
...
static void tcpClient_flush(TRSX *trsx)
{
    #if defined(__AVR__) && defined(__GNUC__)
    	trsx->client->flush();
    #else
...
static void tcpClient_stop(TRSX *trsx)
{
    #if defined(__AVR__) && defined(__GNUC__)
    	trsx->client->stop();
    #else
...
static char httpClient_readChar(TRSX *trsx)
{
    #if defined(__AVR__) && defined(__GNUC__)
...
/******************************************************************************************/
//void http_send_msgbody_asJSON(TRSX *trsx, JSON *json, uint8_t npairs)
//{
//    int i;
//    http_printk(trsx, FS("{"));
//    for (i=0; i<npairs; i++)
...
//httpTrsx_UARTdebug_println(trsx, "IMPRIMIENDO char", 0);
